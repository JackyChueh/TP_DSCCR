var RRS_PVOIData = {
    ...
    Page_Init: function () {
        RRS_PVOIData.EventBinding();
        RRS_PVOIData.OptionRetrieve();
        RRS_PVOIData.ActionSwitch('R');
    },
    ...
    EventBinding: function () {
        ...
        $('#LOCATION').change(function () {
            RRS_PVOIData.SubOptionRetrieve($('#DEVICE_ID'), $(this).val());
        });

        $('#GRAPH_TYPE').change(function () {
            RRS_PVOIData.RRS_PVOIGraph('RRS_PVOI05', $(this).val());
        });

        $('#excel').click(function () {
            RRS_PVOIData.RRS_PVOIExcel();
        });

        $('#login').click(function () {
            window.location.href = RRS_PVOIData.LoginUrl;
        });
    ...
    RRS_PVOIRetrieve: function () {
        var url = 'RRS_PVOIRetrieve';
        var request = {
            RRS_PVOI: {
        ...
        $.ajax({
            ...
            success: function (data) {
                var response = JSON.parse(data);
                RRS_PVOIData.ModalSwitch(response.Result.State);
                if (response.Result.State === 0) {
                    $('#gridview >  tbody').html('');
                    ...
                    if (response.Pagination.RowCount > 0) {
                        $.each(response.RRS_PVOIData, function (idx, row) {
                            htmlRow = '<tr>';
    ...
    RRS_PVOIExcel: function () {
        var url = 'RRS_PVOIExcel';
        var request = {
            RRS_PVOI: {
                LOCATION: $('#LOCATION').val(),
                DEVICE_ID: $('#DEVICE_ID').val()
