var WSDS_PWLSGraph = {
    ...
    Page_Init: function () {
        WSDS_PWLSGraph.EventBinding();
        WSDS_PWLSGraph.OptionRetrieve();
        WSDS_PWLSGraph.ActionSwitch('R');
    ...
    EventBinding: function () {
        ...
        $('#query').click(function () {
            WSDS_PWLSGraph.WSDS_PWLSGraph();
        });

        $('#print').click(function () {
            WSDS_PWLSGraph.WSDS_PWLSGraphPrint();
        });

        $('#LOCATION').change(function () {
            WSDS_PWLSGraph.SubOptionRetrieve($('#DEVICE_ID'), $(this).val());
        });

        $('#login').click(function () {
            window.location.href = WSDS_PWLSGraph.LoginUrl;
        });
    },
    ...
    WSDS_PWLSGraph: function () {
        var url = 'WSDS_PWLSGraph';
        var request = {
            WSDS_PWLS: {
        ...
        $.ajax({
            ...
            success: function (data) {
                var response = JSON.parse(data);
                WSDS_PWLSGraph.ModalSwitch(response.Result.State);
                if (response.Result.State === 0) {
                    if (response.Chart) {
                        if (WSDS_PWLSGraph.ChartObj) {
                            WSDS_PWLSGraph.ChartObj.destroy();
                        }
                        var obj;
                        obj = document.getElementById('chartCanvas').getContext('2d');
                        WSDS_PWLSGraph.ChartObj = new Chart(obj, response.Chart);
                        //WSDS_PWLSGraph.ChartObj.resize();
                    }
                }
                else {
                    $('#modal .modal-title').text('交易訊息');
