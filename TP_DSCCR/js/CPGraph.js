var CPGraph = {
    ...
    Page_Init: function () {
        CPGraph.EventBinding();
        CPGraph.OptionRetrieve();
        CPGraph.ActionSwitch('R');
    ...
    EventBinding: function () {
        ...
        $('#print').click(function () {
            CPGraph.CPGraphPrint();
        });

        $('#LOCATION').change(function () {
            CPGraph.SubOptionRetrieve($('#DEVICE_ID'), $(this).val());
        });

        $('#login').click(function () {
            window.location.href = CPGraph.LoginUrl;
        });
    },
    ...
    CPGraph: function () {
        ...
        $.ajax({
            ...
            success: function (data) {
                var response = JSON.parse(data);
                CPGraph.ModalSwitch(response.Result.State);
                if (response.Result.State === 0) {
                    if (response.Chart) {
                        if (CPGraph.ChartObj) {
                            CPGraph.ChartObj.destroy();
                        }
                        var obj;
                        obj = document.getElementById('chartCanvas').getContext('2d');
                        CPGraph.ChartObj = new Chart(obj, response.Chart);
                        //CPGraph.ChartObj.resize();
                    }
                }
                else {
                    $('#modal .modal-title').text('交易訊息');
