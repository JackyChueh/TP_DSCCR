var CTGraph = {
    ...
    Page_Init: function () {
        CTGraph.EventBinding();
        CTGraph.OptionRetrieve();
        CTGraph.ActionSwitch('R');
    ...
    EventBinding: function () {
        ...
        $('#print').click(function () {
            CTGraph.CTGraphPrint();
        });

        $('#LOCATION').change(function () {
            CTGraph.SubOptionRetrieve($('#DEVICE_ID'), $(this).val());
        });

        $('#login').click(function () {
            window.location.href = CTGraph.LoginUrl;
        });
    },
    ...
    CTGraph: function () {
        ...
        $.ajax({
            ...
            success: function (data) {
                var response = JSON.parse(data);
                CTGraph.ModalSwitch(response.Result.State);
                if (response.Result.State === 0) {
                    if (response.Chart) {
                        if (CTGraph.ChartObj) {
                            CTGraph.ChartObj.destroy();
                        }
                        var obj;
                        obj = document.getElementById('chartCanvas').getContext('2d');
                        CTGraph.ChartObj = new Chart(obj, response.Chart);
                        //CTGraph.ChartObj.resize();
                    }
                }
                else {
                    $('#modal .modal-title').text('交易訊息');
