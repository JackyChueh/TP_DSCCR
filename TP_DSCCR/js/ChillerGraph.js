var ChillerGraph = {
    ...
    Page_Init: function () {
        ChillerGraph.EventBinding();
        ChillerGraph.OptionRetrieve();
        ChillerGraph.ActionSwitch('R');
    ...
    EventBinding: function () {
        ...
        $('#query').click(function () {
            ChillerGraph.ChillerGraph();
        });

        $('#print').click(function () {
            ChillerGraph.ChillerGraphPrint();
        });

        $('#LOCATION').change(function () {
            ChillerGraph.SubOptionRetrieve($('#DEVICE_ID'), $(this).val());
        });

        $('#login').click(function () {
            window.location.href = ChillerGraph.LoginUrl;
        });
    },
    ...
    ChillerGraph: function () {
        var url = 'ChillerGraph';
        var request = {
            Chiller: {
        ...
        $.ajax({
            ...
            success: function (data) {
                var response = JSON.parse(data);
                ChillerGraph.ModalSwitch(response.Result.State);
                if (response.Result.State === 0) {
                    if (response.Chart) {
                        if (ChillerGraph.ChartObj) {
                            ChillerGraph.ChartObj.destroy();
                        }
                        var obj;
                        obj = document.getElementById('chartCanvas').getContext('2d');
                        ChillerGraph.ChartObj = new Chart(obj, response.Chart);
                        //ChillerGraph.ChartObj.resize();
                    }
                }
                else {
                    $('#modal .modal-title').text('交易訊息');
