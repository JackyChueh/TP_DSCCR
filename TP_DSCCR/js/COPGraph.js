var COPGraph = {
    ...
    Page_Init: function () {
        COPGraph.EventBinding();
        COPGraph.OptionRetrieve();
        COPGraph.ActionSwitch('R');
    ...
    EventBinding: function () {
        ...
        $('#print').click(function () {
            COPGraph.COPGraphPrint();
        });

        $('#LOCATION').change(function () {
            COPGraph.SubOptionRetrieve($('#DEVICE_ID'), $(this).val());
        });

        $('#login').click(function () {
            window.location.href = COPGraph.LoginUrl;
        });
    },
    ...
    COPGraph: function () {
        ...
        $.ajax({
            ...
            success: function (data) {
                var response = JSON.parse(data);
                COPGraph.ModalSwitch(response.Result.State);
                if (response.Result.State === 0) {
                    if (response.Chart) {
                        if (COPGraph.ChartObj) {
                            COPGraph.ChartObj.destroy();
                        }
                        var obj;
                        obj = document.getElementById('chartCanvas').getContext('2d');
                        COPGraph.ChartObj = new Chart(obj, response.Chart);
                        //COPGraph.ChartObj.resize();
                    }
                }
                else {
                    $('#modal .modal-title').text('交易訊息');
