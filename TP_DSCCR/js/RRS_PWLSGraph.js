var RRS_PWLSGraph = {
    ...
    Page_Init: function () {
        RRS_PWLSGraph.EventBinding();
        RRS_PWLSGraph.OptionRetrieve();
        RRS_PWLSGraph.ActionSwitch('R');
    ...
    EventBinding: function () {
        ...
        $('#query').click(function () {
            RRS_PWLSGraph.RRS_PWLSGraph();
        });

        $('#print').click(function () {
            RRS_PWLSGraph.RRS_PWLSGraphPrint();
        });

        $('#LOCATION').change(function () {
            RRS_PWLSGraph.SubOptionRetrieve($('#DEVICE_ID'), $(this).val());
        });

        $('#login').click(function () {
            window.location.href = RRS_PWLSGraph.LoginUrl;
        });
    },
    ...
    RRS_PWLSGraph: function () {
        var url = 'RRS_PWLSGraph';
        var request = {
            RRS_PWLS: {
        ...
        $.ajax({
            ...
            success: function (data) {
                var response = JSON.parse(data);
                RRS_PWLSGraph.ModalSwitch(response.Result.State);
                if (response.Result.State === 0) {
                    if (response.Chart) {
                        if (RRS_PWLSGraph.ChartObj) {
                            RRS_PWLSGraph.ChartObj.destroy();
                        }
                        var obj;
                        obj = document.getElementById('chartCanvas').getContext('2d');
                        RRS_PWLSGraph.ChartObj = new Chart(obj, response.Chart);
                        //RRS_PWLSGraph.ChartObj.resize();
                    }
                }
                else {
                    $('#modal .modal-title').text('交易訊息');
