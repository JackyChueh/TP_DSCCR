var WSDS_PVOIData = {
    ...
    Page_Init: function () {
        WSDS_PVOIData.EventBinding();
        WSDS_PVOIData.OptionRetrieve();
        WSDS_PVOIData.ActionSwitch('R');
    },
    ...
    EventBinding: function () {
        ...
        $('#query').click(function () {
            WSDS_PVOIData.WSDS_PVOIRetrieve();
        });
        ...
        $('#LOCATION').change(function () {
            WSDS_PVOIData.SubOptionRetrieve($('#DEVICE_ID'), $(this).val());
        });

        $('#GRAPH_TYPE').change(function () {
            WSDS_PVOIData.WSDS_PVOIGraph('WSDS_PVOI05', $(this).val());
        });

        $('#excel').click(function () {
            WSDS_PVOIData.WSDS_PVOIExcel();
        });

        $('#login').click(function () {
            window.location.href = WSDS_PVOIData.LoginUrl;
        });
    ...
    WSDS_PVOIRetrieve: function () {
        var url = 'WSDS_PVOIRetrieve';
        var request = {
            WSDS_PVOI: {
        ...
        $.ajax({
            ...
            success: function (data) {
                var response = JSON.parse(data);
                WSDS_PVOIData.ModalSwitch(response.Result.State);
                if (response.Result.State === 0) {
                    $('#gridview >  tbody').html('');
                    ...
                    if (response.Pagination.RowCount > 0) {
                        $.each(response.WSDS_PVOIData, function (idx, row) {
                            htmlRow = '<tr>';
    ...
    WSDS_PVOIExcel: function () {
        var url = 'WSDS_PVOIExcel';
        var request = {
            WSDS_PVOI: {
                LOCATION: $('#LOCATION').val(),
                DEVICE_ID: $('#DEVICE_ID').val()
