...
SELECT {0} AS CDATE
    ...
    ,TP_SCC.dbo.PHRASE_NAME('MSPCSTATS_WATER_TOWER',WATER_TOWER,default) AS WATER_TOWER
    ,{1}
    FROM MSPCSTATS
    {2}
    {3}
                ...
                switch (req.GROUP_BY_DT)
                {
                    case "DETAIL":
                        for (int i = 1; i < 9; i++)
                        {
                            ...
                            {
                                fields += "TP_SCC.dbo.PHRASE_NAME('running', CONVERT(DECIMAL(28,0),MSPCSTATS" + i.ToString("00") + "), default) AS SEF" + i.ToString("00") + ",";
                            }
                            else if (i == 8)
                            {
                                fields += "TP_SCC.dbo.PHRASE_NAME('function_fail', CONVERT(DECIMAL(28,0),MSPCSTATS" + i.ToString("00") + "), default) AS SEF" + i.ToString("00") + ",";
                            }
                            else if (i == 9)
                            {
                                fields += "TP_SCC.dbo.PHRASE_NAME('switch_status', CONVERT(DECIMAL(28,0),MSPCSTATS" + i.ToString("00") + "), default) AS SEF" + i.ToString("00") + ",";
                            }
                            else if (i == 10)
                            {
                                fields += "TP_SCC.dbo.PHRASE_NAME('open_close', CONVERT(DECIMAL(28,0),MSPCSTATS" + i.ToString("00") + "), default) AS SEF" + i.ToString("00") + ",";
                            }
                            else
                            {
                                fields += "CONVERT(DECIMAL(28,1),MSPCSTATS" + i.ToString("00") + ") AS SEF" + i.ToString("00") + ",";
        ...
        private MemoryStream ExcelProduce(string GroupByDt, List<MSPCSTATSData> List)
        {
            ...
            using (SpreadsheetDocument SpreadsheetDocument = SpreadsheetDocument.Create(ms, SpreadsheetDocumentType.Workbook))
            {
                ...
                row.Append(
                    ...
                    new Cell()
                    {
                        CellValue = new CellValue("冰水瞬間流量(lpm) (MSPCSTATS及ZP2總管)"),
                        DataType = CellValues.String
                    },
                    new Cell()
                    {
                        CellValue = new CellValue("冰水水路壓差 (MSPCSTATS及ZP2總管)"),
                        DataType = CellValues.String
                    },
                    new Cell()
                    {
                        CellValue = new CellValue("*冰水總出水溫(℃) (MSPCSTATS及ZP2總管)"),
                        DataType = CellValues.String
                    },
                    new Cell()
                    {
                        CellValue = new CellValue("*冰水總回水溫(℃) (MSPCSTATS及ZP2總管)"),
                        DataType = CellValues.String
                    },
                    new Cell()
                    {
                        CellValue = new CellValue("旁通控制閥閥位(%) (MSPCSTATS及ZP2總管)"),
                        DataType = CellValues.String
                    },
                    new Cell()
                    {
...
SELECT {0} AS CDATE
    ,TP_SCC.dbo.PHRASE_NAME('MSPCSTATS_LOCATION',LOCATION,'MSPCSTATS') AS LOCATION
    ,TP_SCC.dbo.PHRASE_NAME('MSPCSTATS_DEVICE_ID',DEVICE_ID,LOCATION) AS DEVICE_ID
    ,TP_SCC.dbo.PHRASE_NAME('MSPCSTATS_WATER_TOWER',WATER_TOWER) AS WATER_TOWER
    ,{1}
    FROM MSPCSTATS
    {2}
    {3}
