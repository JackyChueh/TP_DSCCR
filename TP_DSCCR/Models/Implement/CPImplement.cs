    ...
    public class CPImplement : EnterpriseLibrary
    {
        ...
        public CPDataRes PaginationRetrieve(CPDataReq req)
        {
            ...
            using (DbCommand cmd = Db.CreateConnection().CreateCommand())
            {
                string sql = @"
SELECT {0} AS CDATE
    ,TP_SCC.dbo.PHRASE_NAME('CP_LOCATION',LOCATION,default) AS LOCATION
                ...
                switch (req.GROUP_BY_DT)
                {
                    case "DETAIL":
                        for (int i = 1; i < 8; i++)
                        {
                            if (i == 1)
                            {
                                fields += "TP_SCC.dbo.PHRASE_NAME('running', CONVERT(DECIMAL(28,1),CP" + i.ToString("00") + "), default) AS CP" + i.ToString("00") + ",";
                            }
                            else if (i == 2)
                            {
