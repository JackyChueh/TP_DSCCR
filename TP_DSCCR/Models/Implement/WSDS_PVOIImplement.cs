    ...
    public class WSDS_PVOIImplement : EnterpriseLibrary
    {
        ...
        public WSDS_PVOIDataRes PaginationRetrieve(WSDS_PVOIDataReq req)
        {
            ...
            using (DbCommand cmd = Db.CreateConnection().CreateCommand())
            {
                string sql = @"
SELECT {0} AS CDATE
    ,TP_SCC.dbo.PHRASE_NAME('WSDS_PVOI_LOCATION',LOCATION,default) AS LOCATION
...
";
                ...
                {
                    case "DETAIL":
                        fields += "TP_SCC.dbo.PHRASE_NAME('up_down', CONVERT(DECIMAL(28,1),WSDS_PVOI_STATUS), default) AS WSDS_PVOI_STATUS,";
                        break;
                    case "YEAR":
                    case "MONTH":
                    case "DAY":
